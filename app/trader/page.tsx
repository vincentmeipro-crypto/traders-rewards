        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          {tab === "calendar" && (
            <div>
                ...
                {(["All", "High", "Medium", "Low"] as const).map(f => (
                  <button key={f} onClick={() => setFilter(f)} style={{
                    ...
                    backgroundColor: filter === f
                      ? f === "All" ? "rgba(45,125,210,0.15)" : `${IMPACT_COLOR[f]}22`
                      : "transparent",
                    color: filter === f
                      ? f === "All" ? "#2D7DD2" : IMPACT_COLOR[f]
                      : "#555",
                  }}>
              ...
              {loading ? (
                <div style={{ textAlign: "center", color: "#555", padding: 80 }}>{labels.loading}</div>
              ) : grouped.length === 0 ? (
                <div style={{ textAlign: "center", color: "#555", padding: 80 }}>{labels.noData}</div>
              ) : (
                grouped.map(([date, evs]) => (
                  <div key={date} style={{ marginBottom: 32 }}>
                    ...
                    <div style={{
                      ...
                      borderLeft: "3px solid #2D7DD2", paddingLeft: 12,
                    }}>
                      <span style={{ fontWeight: 800, fontSize: 15, color: "#fff" }}>
                        {formatDate(date, isFr)}
                      </span>
                      <span style={{ color: "#333", fontSize: 12 }}>{evs.length} {isFr ? "annonce(s)" : "event(s)"}</span>
                    </div>
                      ...
                      <div style={{ overflowX: "auto" }}>
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                          <thead>
                            <tr style={{ backgroundColor: "#1a1a24", borderBottom: "1px solid #2A2A38" }}>
                              {[labels.time, labels.currency, labels.event, labels.impact, labels.forecast, labels.previous, labels.actual].map((h, i) => (
                                <th key={i} style={{
                                  padding: "11px 14px", color: "#2D7DD2", fontWeight: 700,
                          ...
                          <tbody>
                            {evs.map((ev, i) => (
                              <tr key={i} style={{
                                borderBottom: i < evs.length - 1 ? "1px solid #111" : "none",
                                ...
                                <td style={{ padding: "11px 14px" }}>
                                  <span style={{
                                    backgroundColor: "rgba(45,125,210,0.15)", color: "#2D7DD2",
                                ...
                                <td style={{ padding: "11px 14px", textAlign: "center" }}>
                                  <span style={{
                                    ...
                                    backgroundColor: IMPACT_COLOR[ev.impact] || "#555",
                                  }} title={ev.impact} />
                                </td>
                                ...
                                <td style={{ padding: "11px 14px", textAlign: "center", fontWeight: 700, color: ev.actual ? "#22c55e" : "#333" }}>
                                  {ev.actual || "—"}
                                </td>
                              </tr>
                            ))}
