...
export default function PromotionsPage() {
  ...
  const toggleActive = async (promo: Promo) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setPromos(prev => prev.map(p => p.id === promo.id ? { ...p, ...data } : p));
  ...
  const deletePromo = async (promo: Promo) => {
    if (promo.used_count > 0) { notify("Impossible : code déjà utilisé", false); return; }
  ...
  const counts = {
    ...
    exhausted: promos.filter(p => p.status === "exhausted").length,
    revoked:   promos.filter(p => p.status === "revoked").length,
  };

  const skeletonWidths = [220, 160, 100, 80, 80, 120, 90, 60];
    ...
    <div style={{ minHeight: "100vh", background: "#050505", color: "#fff", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <a href="/x8k3pz?t=promos" style={{ color: "rgba(255,255,255,0.28)", textDecoration: "none", fontSize: 12 }}
      ...
      <div style={{ padding: "24px 32px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12, marginBottom: 24 }}>
          {([
            { label: "Actives",    value: counts.active,    color: "#4ade80" },
            { label: "Planifiées", value: counts.scheduled, color: "#c084fc" },
            { label: "Expirées",   value: counts.expired,   color: "#fbbf24" },
            { label: "Épuisées",   value: counts.exhausted, color: "#60a5fa" },
            { label: "Révoquées",  value: counts.revoked,   color: "rgba(255,255,255,0.35)" },
          ] as { label: string; value: number; color: string }[]).map(k => (
            <div key={k.label} style={{ background: "#0c0c0c", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "14px 18px" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 680 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {["Code", "Nom", "Remise", "Utilisations", "Statut", "Expire le", "Cible", ""].map(h => (
                      <th key={h} style={{
                    ...
                    return (
                      <tr key={p.id} className="promo-row" style={{
                        ...
                        <td style={{ padding: "14px 16px" }}>
                          <a href={`/x8k3pz/promotions/${p.id}`} style={{
                        ...
                        {/* Nom */}
                        <td style={{ padding: "14px 16px", color: "rgba(255,255,255,0.45)", fontSize: 12, maxWidth: 160 }}>
                          <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {p.name || "—"}
                          </div>
                        </td>
                        {/* Remise */}
                        <td style={{ padding: "14px 16px", fontWeight: 800, color: "#22c55e", fontVariantNumeric: "tabular-nums", fontSize: 14 }}>
                          -{p.discount_percent}%
                        </td>
                        {/* Utilisations */}
                        <td style={{ padding: "14px 16px", fontVariantNumeric: "tabular-nums" }}>
                          <span style={{ color: "#fff", fontWeight: 700 }}>{p.used_count}</span>
                          <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 11, marginLeft: 3 }}>
                            ...
                            <div style={{ position: "relative" }}>
                              ...
                              {menuId === p.id && (
                                <div ref={menuRef} style={{
                                  ...
                                  <button className="action-item" onClick={() => { router.push(`/x8k3pz/promotions/${p.id}`); setMenuId(null); }} style={{
                                  ...
                                  <button className="action-item" onClick={() => toggleActive(p)} style={{
                                    display: "block", width: "100%", textAlign: "left", padding: "8px 12px",
                                    background: "transparent", border: "none", borderRadius: 5,
                                    color: p.active ? "rgba(245,158,11,0.85)" : "#4ade80", fontSize: 12, cursor: "pointer",
                                  }}>
                                    {p.active ? "Révoquer" : "Activer"}
                                  </button>
                                  {p.used_count === 0 && (
                                    <button className="action-item" onClick={() => deletePromo(p)} style={{
                                      display: "block", width: "100%", textAlign: "left", padding: "8px 12px",
                                      background: "transparent", border: "none", borderRadius: 5,
                                      color: "rgba(239,68,68,0.85)", fontSize: 12, cursor: "pointer",
                                    }}>
                                      Supprimer
                                    </button>
                                  )}
