...
export default function AdminCockpitV2(){
 ...
 const load=useCallback(async()=>{setLoading(true);setError("");try{const response=await fetch("/api/admin/overview",{cache:"no-store"});const json=await response.json();if(!response.ok)throw new Error(json.error||"Cockpit indisponible");setData(json)}catch(e){setError(e instanceof Error?e.message:"Erreur inconnue")}finally{setLoading(false)}},[]);
 ...
 return <div className={styles.cockpit}>
  ...
  <section className={styles.grid}>
   <Link href="/x8k3pz?t=pipeline" className={`${styles.card} ${styles.wide}`} style={{textDecoration:"none",color:"inherit"}}><div className={styles.label}>CHALLENGERS ACTIFS</div><div className={`${styles.value} ${styles.blue}`}>{k.activeTotal}</div><div className={styles.sub}>{k.passed} prêt(s) à passer au Compte Reward</div></Link>
   <Link href="/x8k3pz?t=payouts" className={styles.card} style={{textDecoration:"none",color:"inherit"}}><div className={styles.label}>COMPTES REWARD</div><div className={styles.value}>{k.certified}</div><div className={styles.sub}>Comptes Reward actifs</div></Link>
