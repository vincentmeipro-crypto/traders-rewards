...
export default function Rules() {
  ...
  const labels = {
    ...
    tabFunded:    L("Règles Trader Reward","Reglas Trader Reward","Trader Reward Rules"),
    ...
    <section id="rules" className="home-rules" style={{ padding: "80px 24px", backgroundColor: "#000000", position: "relative", overflow: "hidden" }}>
      <div className="home-rules-shell" style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "#D4A843", marginBottom: 16 }}>{T.rules.label}</div>
          <h2 style={{ fontSize: "clamp(2rem, 4vw, 2.8rem)", fontWeight: 800, color: "#FFFFFF", letterSpacing: "-1px" }}>
            {T.rules.title} <span style={{ color: "#D4A843" }}>{T.rules.titleGold}</span>
          </h2>
        </div>
        ...
        <div className="rules-tabs" style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 36 }}>
          {(["challenge", "funded"] as const).map(t => (
            <button key={t} className={`rules-tab${tab === t ? " is-active" : ""}`} onClick={() => setTab(t)} style={{
              ...
              border: tab === t ? "1px solid #D4A843" : "1px solid rgba(255,255,255,0.12)",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 16 }}>
          {groups.map((group, i) => (
            <div key={i} className="rules-surface rules-list-card" style={{ background: "#111111", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, padding: 28 }}>
              <h3 style={{ fontSize: 10, fontWeight: 700, marginBottom: 20, color: i === 0 ? "#D4A843" : "#ef4444", textTransform: "uppercase", letterSpacing: "2px" }}>{group.title}</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {group.items.map((rule, j) => (
                  <div key={j} style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                    <span style={{ color: i === 0 ? "#D4A843" : "#ef4444", fontSize: 14, marginTop: 1, flexShrink: 0, fontWeight: 700 }}>{group.icon}</span>
        ...
        <div className="rules-surface rules-cta" style={{ marginTop: 48, borderRadius: 16, padding: "48px 40px", textAlign: "center", background: "#111111", border: "1px solid rgba(255,255,255,0.1)" }}>
          <h3 style={{ fontSize: "clamp(1.6rem, 3.5vw, 2.2rem)", fontWeight: 800, color: "#FFFFFF", marginBottom: 12, letterSpacing: "-0.5px" }}>
            {T.rules.ctaTitle} <span style={{ color: "#D4A843" }}>{T.rules.ctaGold}</span>
          </h3>
          <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 15, marginBottom: 28 }}>{T.rules.ctaSub}</p>
          <Link href="/#pricing" style={{ display: "inline-block", background: "#D4A843", color: "#000000", padding: "14px 44px", borderRadius: 6, fontWeight: 800, fontSize: 12, letterSpacing: "2px", textTransform: "uppercase", textDecoration: "none", transition: "all 0.2s" }}
