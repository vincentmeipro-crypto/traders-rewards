    ...
    <section id="pricing" style={{ padding: "80px 24px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          <span style={{ color: "#C9A84C", fontSize: 12, fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", display: "block", marginBottom: 16 }}>{T.pricing.label}</span>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 48 }}>
          <div style={{ backgroundColor: "#111", border: "1px solid #222", borderRadius: 12, padding: 4, display: "flex", gap: 4 }}>
            {(["2step", "1step"] as const).map(m => (
              <button key={m} onClick={() => setModel(m)} style={{
                padding: "10px 28px", borderRadius: 8, border: "none", cursor: "pointer",
                fontSize: 14, fontWeight: 700, letterSpacing: "0.5px", transition: "all 0.2s",
                backgroundColor: model === m ? "#C9A84C" : "transparent",
        ...
        <div style={{ backgroundColor: "#111", border: "1px solid #1a1a1a", borderRadius: 16, padding: "24px 32px", marginBottom: 40, maxWidth: 800, margin: "0 auto 40px" }}>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", justifyContent: "center" }}>
          {accounts.map((acc, i) => (
            <div key={i} className={acc.popular ? "card-featured" : "card"} style={{ padding: "32px 24px", textAlign: "center", flex: "1 1 180px", maxWidth: 220, position: "relative" }}>
              {acc.popular && (
                <div style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", backgroundColor: "#C9A84C", color: "#000", fontSize: 11, fontWeight: 800, padding: "4px 16px", borderRadius: 100, letterSpacing: "1px", textTransform: "uppercase", whiteSpace: "nowrap" }}>
                  {T.pricing.mostPopular}
                </div>
              )}
              <div style={{ color: "#666", fontSize: 12, marginBottom: 8, textTransform: "uppercase", letterSpacing: "1px" }}>{T.pricing.account}</div>
              <div style={{ fontSize: 26, fontWeight: 900, color: "#C9A84C", marginBottom: 4 }}>{acc.size}</div>
