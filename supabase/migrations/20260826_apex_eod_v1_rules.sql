...
-- rewards-25k  :  250 /  375 /  500 /  625 /   750 USD
-- rewards-50k  :  500 /  750 / 1000 / 1250 /  1500 USD
-- rewards-100k : 1000 / 1500 / 2000 / 2500 /  3000 USD
-- ⚠ Migration historique déjà exécutée en DB — utiliser ON CONFLICT DO UPDATE pour mettre à jour.
-- Pour appliquer ces nouvelles valeurs en production, re-exécuter ce bloc §3 dans Supabase SQL Editor.

INSERT INTO challenge_product_rules (product_id, rule_key, rule_value, enabled, description)
VALUES
  -- 25K
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-25k'),  'reward_cap_1',  '250'::jsonb,  true, 'Plafond Reward #1 — 25K : 250 USD. (1 % de 25 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-25k'),  'reward_cap_2',  '375'::jsonb,  true, 'Plafond Reward #2 — 25K : 375 USD. (1,5 % de 25 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-25k'),  'reward_cap_3',  '500'::jsonb,  true, 'Plafond Reward #3 — 25K : 500 USD. (2 % de 25 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-25k'),  'reward_cap_4',  '625'::jsonb,  true, 'Plafond Reward #4 — 25K : 625 USD. (2,5 % de 25 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-25k'),  'reward_cap_5',  '750'::jsonb,  true, 'Plafond Reward #5 — 25K : 750 USD. (3 % de 25 000)'),
  -- 50K
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-50k'),  'reward_cap_1',  '500'::jsonb,  true, 'Plafond Reward #1 — 50K : 500 USD. (1 % de 50 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-50k'),  'reward_cap_2',  '750'::jsonb,  true, 'Plafond Reward #2 — 50K : 750 USD. (1,5 % de 50 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-50k'),  'reward_cap_3', '1000'::jsonb,  true, 'Plafond Reward #3 — 50K : 1 000 USD. (2 % de 50 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-50k'),  'reward_cap_4', '1250'::jsonb,  true, 'Plafond Reward #4 — 50K : 1 250 USD. (2,5 % de 50 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-50k'),  'reward_cap_5', '1500'::jsonb,  true, 'Plafond Reward #5 — 50K : 1 500 USD. (3 % de 50 000)'),
  -- 100K
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-100k'), 'reward_cap_1', '1000'::jsonb,  true, 'Plafond Reward #1 — 100K : 1 000 USD. (1 % de 100 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-100k'), 'reward_cap_2', '1500'::jsonb,  true, 'Plafond Reward #2 — 100K : 1 500 USD. (1,5 % de 100 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-100k'), 'reward_cap_3', '2000'::jsonb,  true, 'Plafond Reward #3 — 100K : 2 000 USD. (2 % de 100 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-100k'), 'reward_cap_4', '2500'::jsonb,  true, 'Plafond Reward #4 — 100K : 2 500 USD. (2,5 % de 100 000)'),
  ((SELECT id FROM challenge_products WHERE slug = 'rewards-100k'), 'reward_cap_5', '3000'::jsonb,  true, 'Plafond Reward #5 — 100K : 3 000 USD. (3 % de 100 000)')
ON CONFLICT (product_id, rule_key) DO UPDATE
  SET rule_value  = EXCLUDED.rule_value,
      enabled     = EXCLUDED.enabled,
      description = EXCLUDED.description;
