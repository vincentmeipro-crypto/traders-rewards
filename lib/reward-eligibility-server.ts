...
export async function loadRewardAccounts(db: SupabaseClient, userId: string, onlyId?: string) {
  ...
  // Fetch paid_at + created_at pour le calcul du cycle (reset depuis le dernier paiement)
  const [accounts, payouts, profile] = await Promise.all([
    query,
    db.from("payouts").select("challenge_id,status,paid_at,created_at").eq("user_id", userId),
  ...
  for (const c of accounts.data ?? []) {
    ...
    const paidCount  = related.filter(p => p.status === "paid").length;

    const base = {
      start:        Number(c.start_balance),
      balance:      Number(c.balance),
      equity:       Number(c.balance),
      phase:        c.phase,
      status:       c.status,
      paidCount,
      terminated:   Boolean(c.terminated_at || c.breach_at),
      pending:      related.some(p => ["pending", "approved", "processing"].includes(p.status)),
      kyc:          profile.data?.kyc_status === "approved",
      dailyProfits: [] as number[],
    };
    ...
    try {
      ...
      // Cycle = depuis paid_at du dernier Reward payé (reset consistency / jours qualifiants).
      // Si aucune Reward payée : depuis reward_converted_at (activation du Reward Account).
      // Fallback paid_at → created_at si paid_at absent (avant migration 20260911).
      ...
      for (const raw of history) {
        ...
        const time       = Number.isFinite(numericTime)
          ? (numericTime > 1e10 ? numericTime : numericTime * 1000)
          : Date.parse(String(rawTime));
        if (!Number.isFinite(time)) throw new Error("Missing deal time");
        if (time < since) continue; // Hors cycle courant → ignorer
      ...
      eligibility = {
        ...
        eligible: false,
        maximum:  0,
        reasons:  [...eligibility.reasons, "unavailable"],
      };
    }
    results.push({
      id:          c.id as string,
      accountSize: c.account_size as string,
      login:       c.mt5_login as number | null,
      ...eligibility,
    });
  }
  return results;
