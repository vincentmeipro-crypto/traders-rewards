...
// ─── Suite 3 : Calculateur de Lot (18 cas) ───────────────────────────────────

const lotResults = runLotTests();

// ─── Suite 4 : Simulateur de Risque (25 cas) ─────────────────────────────────

const simResults = runSimTests();
...
printResults("Simulateur de Risque (perte SL · DD · statut)", simResults);

const totalTests = TEST_CASES.length + percentageTests.length + LOT_TEST_CASES.length + SIM_TEST_CASES.length;
// ↑ Attendu : 8 instruments + 4 pourcentages + 18 lot + 25 simulateur = 55 tests
console.log(`\n${BOLD}───────────────────────────────────────────────────────${RESET}`);
